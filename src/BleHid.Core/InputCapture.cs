...
public sealed class InputCapture : IDisposable
{
    ...
    private int _resetInputState;
    private volatile bool _passThrough;
    private volatile bool _running;
    private EdgeSwitchDetector? _edgeDetector;
    private IntPtr _edgeTimer;
    private int _edgeSamples;
    internal int EdgeSamples => Volatile.Read(ref _edgeSamples);
    ...
    public event Action? EdgeSwitchRequested;
    ...
    public void SetPassThrough(bool value)
    {
        ...
        if (value && _running && _localCursor is { } point) SetCursorPos(point.X, point.Y);
    }
    ...
    public void Start()
    {
        ...
        try
        {
            _started.Task.WaitAsync(TimeSpan.FromSeconds(5)).GetAwaiter().GetResult();
        }
        catch
        {
            Stop();
    ...
    public void Stop()
    {
        // Restore local control before waiting for the hook thread to exit.
        SetPassThrough(true);
        _running = false;
    ...
    private void HookThread()
    {
        ...
        finally
        {
            _passThrough = true;
            _running = false;
            if (_edgeTimer != IntPtr.Zero) KillTimer(IntPtr.Zero, _edgeTimer);
            _edgeTimer = IntPtr.Zero;
    ...
    private void RunHookLoop()
    {
        ...
        if (_keyboardHook == IntPtr.Zero)
            throw new Win32Exception(keyboardError, "Could not install the keyboard hook; input remains on this PC.");
        if (_mouseHook == IntPtr.Zero)
            throw new Win32Exception(mouseError, "Could not install the mouse hook; input remains on this PC.");
        ...
        if (_winEventHook == IntPtr.Zero)
            throw new Win32Exception(Marshal.GetLastWin32Error(), "Could not monitor foreground changes; input remains on this PC.");

        if (EdgeSwitch is { } edge)
        {
            // A low-level mouse callback is not a dwell clock: at the desktop limit there
            // may be no further moves, and callback coordinates can precede cursor clipping.
            // Sample the actual clipped cursor on this same thread, even while stationary.
        ...
        var result = 0;
        while (_running && (result = GetMessage(out var message, IntPtr.Zero, 0, 0)) > 0)
        {
            if (message.message == 0x0113 /* WM_TIMER */ && message.wParam == _edgeTimer && _edgeTimer != IntPtr.Zero)
            {
                PollEdgeCursor();
                continue;
            }
        ...
        if (result == -1)
            throw new Win32Exception(Marshal.GetLastWin32Error(), "The input capture message loop failed.");
    ...
    private void ForegroundChanged(IntPtr hook, uint eventType, IntPtr window, int idObject, int idChild, uint thread, uint time)
    {
        ...
        {
            FailCapture(new Win32Exception(_keyboardHook == IntPtr.Zero ? keyboardError : mouseError,
                "Could not restore input hooks after a focus change; input remains on this PC."));
            return;
        }
        _rearms++;
    ...
    {
        if (code != HC_ACTION || !_running) return CallNextHookEx(IntPtr.Zero, code, wParam, lParam);
    ...
    private IntPtr MouseHookProc(int code, IntPtr wParam, IntPtr lParam)
    {
        ...
        var gesture = _middleReturn.Handle((int)wParam, !_passThrough, ReturnOptions.MiddleClick);
        if (gesture == ReturnGesture.ReturnLocal)
        {
            Log?.Invoke("[telas] Clique da rodinha: voltando ao PC.");
        ...
        switch ((int)wParam)
        {
            case WM_MOUSEMOVE:
                ...
                if (dx != 0 || dy != 0)
                {
                    var held = _buttons != MouseButtons.None || _pressedVirtualKeys.Count > 0;
                    if (_remoteEstimate?.Move(dx, dy, held, Environment.TickCount64) == true)
                    {
                        Log?.Invoke("[telas] Borda estimada: voltando ao PC.");
                        ReturnLocalRequested?.Invoke();
                        return 1;
                    }
    ...
    {
        if (!_running || !_passThrough || _edgeDetector is null) return;
    ...
    // where their key-up/button-up events may deliberately have passed through untouched.
    ...
    [DllImport("user32.dll", SetLastError = true)]
    private static extern IntPtr SetTimer(IntPtr window, IntPtr id, uint intervalMs, IntPtr callback);
